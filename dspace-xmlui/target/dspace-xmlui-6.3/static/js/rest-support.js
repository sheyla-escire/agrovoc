...
(function($) {
    $(document).ready(function(){
        ...
        var vocabularyUrls = $('a[href^="rest:"]');
        vocabularyUrls.click(function(index){
            var $link = $(this);
            var vocabularyJSONUrl = $link.attr('href').replace('rest:', '');
            //Retrieve the basic url, we will need this to add images !
            var basicUrl = vocabularyJSONUrl.substr(0, vocabularyJSONUrl.indexOf('/controlled-rest'));
            ...
            if(0 < vocabularyDialog.length){
                ...
                $.get(basicUrl + '/controlled-rest-dialog',
                    {
                        restIdentifier: vocabularyIdentifier,
                        metadataFieldName: inputFieldName
                    },
                    function(resultingHtml){
                        // alert("Data: " + resultingHtml );
